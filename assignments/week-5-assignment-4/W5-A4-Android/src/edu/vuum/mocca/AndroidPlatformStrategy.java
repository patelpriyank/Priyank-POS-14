...
{	
    ...
    {	
    	...
    	//if activity is dead already then just decrement mLatch count on current thread
    	if(activity == null) 
    		mLatch.countDown();
    	
    	//otherwise run mLatch.countDown() on UI thread to ensure that it runs last 
    	//after all other work is completed on UI thread
    	activity.runOnUiThread(new Runnable() {
